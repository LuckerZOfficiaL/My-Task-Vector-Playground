...
def run(cfg: DictConfig) -> str:
    ...
    for dataset_name in cfg.eval_datasets:
        ...
        model = hydra.utils.instantiate(
            cfg.nn.module, encoder=zeroshot_model, classifier=classification_head, _recursive_=False
        )
    ...
    if cfg.export_zs_evals:
        results_export_dir = "./evaluations/evaulate_zs"
        os.makedirs(results_export_dir, exist_ok=True)
        export_dict_to_json(
            data=results, 
            filename=os.path.join(results_export_dir, f"results_zs.json"),
            export_description="evaluation zs results"
        )
    else:
        pylogger.info(f"cfg.export_zs_evals = {cfg.export_zs_evals}, results NOT exported.")
