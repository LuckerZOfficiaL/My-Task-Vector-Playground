...
CORRELATION_PLOTS_EXPORT_DIR = f"./plots/correlations/delta_TA_ATM"
...
def _populate_list_of_task_equipped_models():
    ...
    list_of_task_equipped_models_ta = [
        model for model in list_of_task_equipped_models if TA_STR in model
    ]

    list_of_task_equipped_models_atm = [
        model for model in list_of_task_equipped_models if ATM_STR in model
    ]

    list_of_task_equipped_models_ta = sorted(list_of_task_equipped_models_ta)
    list_of_task_equipped_models_atm = sorted(list_of_task_equipped_models_atm)

    list_of_task_equipped_models = []

    for x, y in zip(list_of_task_equipped_models_ta, list_of_task_equipped_models_atm):
        if not x.replace("epochs_TA", "") == y.replace("epochs_1", ""):
            raise ValueError(f"TA model {x} and ATM model {y} do not match for another parameter than epochs.")
        
        list_of_task_equipped_models.append([x, y])
...
def _populate_metrics_for_task_equipped_models(
    datasets_metrics: dict,
    list_of_task_equipped_models: List[List[str]]
):

    task_equipped_models_metrics = {}

    for task_equipped_model_ta, task_equipped_model_atm in list_of_task_equipped_models:

        if not task_equipped_model_ta.replace("epochs_TA", "") == task_equipped_model_atm.replace("epochs_1", ""):
            raise ValueError(f"TA model {task_equipped_model_ta} and ATM model {task_equipped_model_atm} do not match for another parameter than epochs.")

        task_equipped_model = task_equipped_model_ta.replace("epochs_TA", "")

        merged_accs_full_dict_ta = load_dict_from_json(
            filename=os.path.join(TASK_EQUIPPED_ACCS_DIR, task_equipped_model_ta),
            import_description="task-equipped accuracies TA"
        )

        merged_accs_ta = merged_accs_full_dict_ta["dataset_evals"]
        # print(f"merged accuracies")
        # pprint(merged_accs_ta)

        normalized_merged_accs_ta = _compute_normalized_merged_accs(
            datasets_metrics=datasets_metrics,
            merged_accs=merged_accs_ta
        )
        # print(f"normalized merged accuracies")
        # pprint(normalized_merged_accs_ta)
        
        merged_accs_full_dict_atm = load_dict_from_json(
            filename=os.path.join(TASK_EQUIPPED_ACCS_DIR, task_equipped_model_atm),
            import_description="task-equipped accuracies TA"
        )

        merged_accs_atm = merged_accs_full_dict_atm["dataset_evals"]
        # print(f"merged accuracies")
        # pprint(merged_accs_atm)

        normalized_merged_accs_atm = _compute_normalized_merged_accs(
            datasets_metrics=datasets_metrics,
            merged_accs=merged_accs_atm
        )
        # print(f"normalized merged accuracies")
        # pprint(normalized_merged_accs_atm)

        if normalized_merged_accs_ta.keys() == normalized_merged_accs_atm.keys():
            applied_task_vectors = normalized_merged_accs_ta.keys()
        else:
            raise ValueError(
                f"Normalized merged accuracies for TA and ATM do not match.\n"
                f"TA : {normalized_merged_accs_ta.keys()}\n"
                f"ATM: {normalized_merged_accs_atm.keys()}"
            )

        delta_normalized_merged_accs = {
            dataset_name: normalized_merged_accs_ta[dataset_name] - normalized_merged_accs_atm[dataset_name]
            for dataset_name in applied_task_vectors
        }
        ...
        task_equipped_models_metrics[task_equipped_model] = {
            "delta_normalized_merged_accs": delta_normalized_merged_accs,
            "acc_gap": acc_gap,
            "acc_ratio": acc_ratio,
            "loss_gap": loss_gap,
            "normalized_loss_gap": normalized_loss_gap
...
def plot_or_export(
    ...
    plt.ylabel('Delta Normalized Merged Accuracy (higher is better)', fontsize=12)
...
def _plot_correlation(
    delta_normalized_merged_accs: dict,
    metric: dict,
    metric_name: str,
    metric_y_axis_label: str,
    plot_title: str,
    ...
    # pprint(delta_normalized_merged_accs)

    # print(f"metric")
    # pprint(metric)

    if len(delta_normalized_merged_accs) != len(metric):
        raise ValueError(
            f"The number of datasets in the normalized merged accuracies {len(delta_normalized_merged_accs)} and the metric {len(metric)} must be the same."
        )

    avg_merged_accs  = {}

    for subset_name, merged_acc_dict in delta_normalized_merged_accs.items():
...
def _plot_correlations(
    ...
    delta_normalized_merged_accs = {
        model: task_equipped_metrics[model]["delta_normalized_merged_accs"]
    ...
    _plot_correlation(
        ...
        plot_title="acc gap, delta normalized merged accs correlation",
        export_path=export_path
    )
    ...
    _plot_correlation(
        ...
        plot_title="acc ratio, delta normalized merged accs correlation",
        export_path=export_path
    )
    ...
    _plot_correlation(
        ...
        plot_title="loss gap, delta normalized merged accs correlation",
        export_path=export_path
    )
    ...
    _plot_correlation(
        ...
        plot_title="normalized loss gap, delta normalized merged accs correlation",
        export_path=export_path
    )
...
def main():
    ...
    print(f"Number of task equipped model pairs: {len(list_of_task_equipped_models)}")
