...
if [[ "$1" == "--timestamp" ]]; then
    ...
    # Run with timestamp
    python3 src/scripts/autoexperiment.py --timestamp=$timestamp --called-from-bash-script=True | tee ./run_terminal_logs/autoexperiment/autoexperiment_output_log_$timestamp.log
else
