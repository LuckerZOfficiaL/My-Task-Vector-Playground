...
# test_dataset --> val split (testdir = "val")
# 
# the original test split is not used, probably intended to be used as validation...
# 
# For this reason, we set valdir = "test".
class EuroSATOurNeeds():
    def __init__(self, preprocess, location="~/datasets", batch_size=32, num_workers=16):
        # Data loading code
        traindir = os.path.join(location, "EuroSAT_splits", "train")
        testdir = os.path.join(location, "EuroSAT_splits", "val")
...
class EuroSATVal(EuroSATOurNeeds):
    def __init__(self, preprocess, location="~/datasets", batch_size=32, num_workers=16):
